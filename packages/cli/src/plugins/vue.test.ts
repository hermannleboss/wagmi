...
test('default', async () => {
  ...
  expect(result?.imports).toMatchInlineSnapshot(`
    "import { createUseReadContract, createUseWriteContract, createUseSimulateContract, createUseWatchContractEvent } from '@wagmi/vue/codegen'
    "
  `)
...
test('legacy composable names', async () => {
  const result = await vue({ getComposableName: 'legacy' }).run?.({
    contracts: [
      {
        name: 'erc20',
        abi: erc20Abi,
